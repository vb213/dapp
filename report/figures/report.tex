...
\draw[arrow] (mm.south) to[bend right=20] (hub.north);
\draw[arrow] (hub) -- (dex);
\draw[arrow] (hub) -- (nft);
...
A \texttt{README.md} and a deploy script automate the setup. Four terminals are required: the Hardhat node, the deployment script (which seeds liquidity and copies the ABIs to the frontend), the metadata server and the static frontend served on port 8080. The deployment also transfers ownership of \texttt{DexToken} to \texttt{PawningHub} so requirement~6 can be enforced from a single contract. The contract suite is covered by 27 Hardhat tests organised by feature (\texttt{dex}, \texttt{nft}, \texttt{hub-dex-loan}, \texttt{hub-market}, \texttt{hub-nft-loan}, \texttt{hub-admin}); all of them pass. The full system runs locally on Hardhat; deploying to a public testnet only requires changing the network configuration.
